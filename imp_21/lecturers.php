...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add_lecturer') {
    ...
    if ($name === '' || $email === '') {
        ...
        try {
            $stmt = $pdo->prepare("INSERT INTO users (name, email, password, role, code, major) VALUES (?, ?, ?, 'lecturer', ?, ?)");
            $stmt->execute([$name, $email, $password, $code !== '' ? $code : null, $major !== '' ? $major : null]);
            $message = "Đã thêm giảng viên. GV sẽ hiện trong bảng sau khi được phân công sinh viên ở trang Assignments.";
        } catch (PDOException $e) {
            $type = "error";
            $message = "Không thể thêm. Email có thể đã tồn tại.";
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'edit_lecturer') {
    ...
    if ($user_id <= 0 || $name === '' || $email === '') {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE users SET name = ?, email = ?, code = ?, major = ? WHERE user_id = ? AND role = 'lecturer'");
...
if ($selectedPeriod > 0) {
    $stmt = $pdo->prepare("
        ...
        WHERE ir.period_id = ?
        GROUP BY lec.user_id, lec.name, lec.email, lec.code, lec.major
        ORDER BY lec.name
    ");
