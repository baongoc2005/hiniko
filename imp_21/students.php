...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add_student') {
    ...
    if ($name === '' || $email === '' || $period_id <= 0) {
        $type = "error";
        $message = "Vui lòng nhập đầy đủ họ tên, email.";
    } else {
        try {
            $pdo->beginTransaction();

            $stmt = $pdo->prepare("INSERT INTO users (name, email, password, role, code, major) VALUES (?, ?, ?, 'student', ?, ?)");
            $stmt->execute([$name, $email, $password, $code !== '' ? $code : null, $major !== '' ? $major : null]);
            ...
            $pdo->rollBack();
            $type = "error";
            $message = "Không thể thêm. Email có thể đã tồn tại hoặc dữ liệu không hợp lệ.";
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'edit_student') {
    ...
    if ($user_id <= 0 || $name === '' || $email === '') {
        ...
        try {
            ...
            $stmt = $pdo->prepare("UPDATE users SET name = ?, email = ?, code = ?, major = ? WHERE user_id = ? AND role = 'student'");
            $stmt->execute([$name, $email, $code !== '' ? $code : null, $major !== '' ? $major : null, $user_id]);
...
$lecturers = $pdo->query("SELECT user_id, name, code FROM users WHERE role = 'lecturer' ORDER BY name")->fetchAll();
...
if ($selectedPeriod > 0) {
    $stmt = $pdo->prepare("
        ...
        WHERE ir.period_id = ?
        GROUP BY ir.ir_id, ir.position_id, ir.status, u.user_id, u.name, u.email, u.code, u.major, ip.title, c.name, lec.name, ia.lecturer_id
        ORDER BY u.name
    ");
