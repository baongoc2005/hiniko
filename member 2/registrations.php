...
$pageTitle = "Internship Registrations";
    ...
    if ($mode === 'propose') {
        ...
        try {
            $matchStmt = $pdo->prepare("
                SELECT u.major AS student_major, ip.major AS position_major
                FROM users u, internship_positions ip
                WHERE u.user_id = ? AND ip.ip_id = ?
            ");
...
if ($isStudent) {
    $regStmt = $pdo->prepare("
        SELECT ir.*, u.name AS student_name, u.code AS student_code, ip.title, c.name AS company_name, p.name AS period_name
...
<div style="display:flex; justify-content:space-between; align-items:center; gap:16px; flex-wrap:wrap;">
    <div>
        <h1 style="font-size:26px; font-weight:700; color:#1e293b; margin:0;">Đăng ký thực tập</h1>
        <p class="help" style="margin:4px 0 0;">Tổng cộng <?= count($registrations) ?> đăng ký.</p>
    </div>
        ...
        <form method="POST">
            ...
            <?php if ($isStudent): ?>
                <div class="form-group">
                    ...
                    <input type="text" value="<?= e(currentUser()['name'] ?? '') ?> (<?= e(currentUser()['major'] ?? 'chưa có ngành') ?>)" disabled>
                </div>
                <div class="form-group">
                    <label>Mã sinh viên</label>
                    <input type="text" value="<?= e(currentUser()['code'] ?? '') ?>" disabled placeholder="Chưa có mã">
                </div>
            <?php else: ?>
                <div class="form-group">
                    <label>Sinh viên *</label>
                    <select name="student_id" id="regStudentSelect" required onchange="syncStudentCode()">
                        <?php foreach ($students as $s): ?>
                            <option value="<?= e($s['user_id']) ?>" data-code="<?= e($s['code'] ?? '') ?>"><?= e($s['name']) ?> — <?= $s['code'] ? e($s['code']) : 'Chưa có mã' ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group">
        ...
        <form method="POST">
            ...
            <div class="form-group">
                ...
                <select name="student_id" id="editStudentId" required>
                    <?php foreach ($students as $s): ?>
                        <option value="<?= e($s['user_id']) ?>"><?= e($s['name']) ?> — <?= $s['code'] ? e($s['code']) : 'Chưa có mã' ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="form-group">
