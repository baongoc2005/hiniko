...
$studentBase = "
    SELECT ir.ir_id, u.name AS student_name, u.code AS student_code,
...
if ($isStudent) {
    $sStmt = $pdo->prepare($studentBase . " WHERE ir.student_id = ? GROUP BY ir.ir_id, u.name, u.code, ip.title, c.name ORDER BY u.name");
    ...
    $sStmt = $pdo->prepare($studentBase . "
        ...
        GROUP BY ir.ir_id, u.name, u.code, ip.title, c.name ORDER BY u.name");
    $sStmt->execute([currentUserId()]);
    $studentRows = $sStmt->fetchAll();
} else {
    $studentRows = $pdo->query($studentBase . " GROUP BY ir.ir_id, u.name, u.code, ip.title, c.name ORDER BY u.name")->fetchAll();
}
...
<div style="display:flex; justify-content:space-between; align-items:center; gap:16px; flex-wrap:wrap;">
    <h1 style="font-size:26px; font-weight:700; color:#1e293b; margin:0;">Nhật ký hàng tuần</h1>
...
<?php else: ?>
<h1 style="font-size:26px; font-weight:700; color:#1e293b; margin:0;">Báo cáo nhật ký hàng tuần</h1>
<p class="help" style="margin:4px 0 0;"><?= $isLecturer ? 'Sinh viên bạn phụ trách' : 'Toàn bộ sinh viên' ?> — bấm vào sinh viên để xem chi tiết 6 tuần.</p>
<?php endif; ?>
    ...
    <div class="modal-content">
        ...
        ?>
            <div class="week-item">
                ...
                <?php endif; ?>
                <?php if ($f): ?>
                    <p style="margin-top:8px;"><a class="btn small" href="<?= e($f['file_path']) ?>" target="_blank">📄 Tải file báo cáo (<?= e($f['file_type']) ?>)</a></p>
                <?php endif; ?>
