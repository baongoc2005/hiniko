...
if ($isLecturer) {
    $aStmt = $pdo->prepare("
        SELECT ia.*, stu.name AS student_name, s.student_code, s.major AS student_major, lec.name AS lecturer_name, ip.title, c.name AS company_name
        ...
        LEFT JOIN students s ON s.user_id = stu.user_id
